...
export async function generatePageContent(site, pageConfig) {
  ...
  const mainCountry = biz.country || 'CH';
  const mainCountryLabel = mainCountry === 'CH' ? 'suisses' : 'françaises';
  const sysPrompt = `Tu es un rédacteur web SEO expert pour entreprises locales ${mainCountryLabel}. Style fluide et engageant. Paragraphes de 3-4 phrases. Chaque phrase apporte de la valeur. Pas de remplissage. Réponds UNIQUEMENT en JSON valide.`;
...
export async function generateBookingPageContent(site, config) {
  ...
  const country = biz.country || 'CH';
  const currency = country === 'CH' ? 'CHF' : '€';
  const countryLabel = country === 'CH' ? 'Suisse' : 'France';

  const sysPrompt = `Tu es un rédacteur web expert pour praticiens et professionnels de santé/bien-être/services en ${countryLabel}. Tu crées du contenu engageant pour une page de réservation en ligne. Style professionnel, rassurant et chaleureux. Réponds UNIQUEMENT en JSON valide.`;
...
Génère du contenu pour une page de réservation de praticien. Les prestations doivent avoir des durées et prix RÉALISTES pour la spécialité "${activity}" en ${countryLabel}. Utilise la devise ${currency}. JSON:
{"heroPractitioner":{"name":"${name}","specialty":"${activity}","tagline":"accroche 1-2 phrases rassurante et professionnelle","ctaText":"Prendre rendez-vous","ctaUrl":"#booking"},"servicesBooking":{"title":"Nos prestations","subtitle":"courte description engageante","services":[{"name":"prestation principale","duration":"60 min","price":"XX ${currency}","description":"2-3 phrases décrivant la prestation"},{"name":"prestation 2","duration":"45 min","price":"XX ${currency}","description":"2-3 phrases"},{"name":"prestation 3","duration":"30 min","price":"XX ${currency}","description":"2-3 phrases"},{"name":"prestation 4","duration":"90 min","price":"XX ${currency}","description":"2-3 phrases"}]},"about":{"title":"À propos de ${name}","body":"<p>3-4 phrases parcours et formation.</p><p>2-3 phrases philosophie et approche.</p>","bulletPoints":[{"value":"diplôme ou certification"},{"value":"années d'expérience"},{"value":"spécialisation"},{"value":"approche unique"}]},"googleReviews":{"title":"Avis Google","testimonials":[{"text":"avis réaliste 2-3 phrases sur l'expérience","name":"Prénom L.","rating":5},{"text":"avis réaliste 2-3 phrases","name":"Prénom M.","rating":5},{"text":"avis réaliste 2-3 phrases","name":"Prénom D.","rating":4}],"ctaText":"Voir nos avis"},"faq":{"items":[{"question":"Comment se déroule une première consultation ?","answer":"réponse 3-4 phrases"},{"question":"Quels sont les tarifs ?","answer":"réponse 2-3 phrases"},{"question":"Comment annuler ou reporter un rendez-vous ?","answer":"réponse 2-3 phrases"},{"question":"question spécifique à ${activity}","answer":"réponse 2-3 phrases"}]},"contact":{"title":"${name}${city ? ' — ' + city : ''}","body":"<p>Adresse et informations pratiques</p>","hours":"${biz.hours || 'Du lundi au vendredi de 9h à 19h'}"},"seo":{"title":"${name} — ${activity}${city ? ' à ' + city : ''} | Réservation en ligne","description":"Prenez rendez-vous avec ${name}, ${activity.toLowerCase()}${city ? ' à ' + city : ''}. Réservation en ligne simple et rapide.","keywords":["${activity.toLowerCase()} ${city.toLowerCase()}","rendez-vous ${activity.toLowerCase()}","réservation en ligne ${city.toLowerCase()}","${name.toLowerCase()}","praticien ${city.toLowerCase()}"]}}`;
